...
class MSSDataset(torch.utils.data.Dataset):
            ...
            if read_metadata_procs <= 1:
                for path in tqdm(track_paths):
                    track_path, track_length = get_track_set_length(
                        (path, self.instruments, self.file_types)
                    )
    ...
    def augm_data(self, source, instr):
        ...
        if "pedalboard_compressor" in augs:
            if augs["pedalboard_compressor"] > 0:
                if random.uniform(0, 1) < augs["pedalboard_compressor"]:
                    board = PB.Pedalboard(
                        [
                            PB.Compressor(
                                threshold=augs["pedalboard_compressor_threshold"],
                                ratio=augs["pedalboard_compressor_ratio"],
                                attack=augs["pedalboard_compressor_attack"],
                                release=augs["pedalboard_compressor_release"],
                            )
                        ]
                    )
                    source = board(source, 44100)
                    applied_augs.append("pedalboard_compressor")
        # Random Bitcrash
