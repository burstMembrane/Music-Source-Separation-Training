...
def parse_args(dict_args: Union[Dict, None]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        help="Model type for secondary embedding model",
    )
    parser.add_argument(
        "--secondary_config_path", type=str, help="Path to config for secondary model"
    )
    parser.add_argument(
        "--secondary_checkpoint", type=str, help="Checkpoint path for secondary model"
    )
...
def wandb_init(
    ...
    if args.wandb_key is None or args.wandb_key.strip() == "":
        ...
        wandb.init(
            project="hstasnet_pretrained_emb",
            config={
                "config": config,
                "args": args,
...
def train_one_epoch(
    ...
    for i, (batch, mixes) in enumerate(pbar):
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            # Get embeddings from secondary model
            secondary_model.eval()  # Ensure secondary model is in eval mode
            with torch.no_grad():
                target_embeddings = secondary_model(
                    y
                ).detach()  # Get embeddings for target
            ...
            if args.model_type in ["mel_band_roformer", "bs_roformer"]:
                ...
                y_ = model(x)  # Original forward pass without embeddings
                ...
                # Get embeddings for model output
                secondary_model.eval()  # Ensure secondary model is in eval mode
                with torch.no_grad():
                    pred_embeddings = secondary_model(
                        y_
                    ).detach()  # Get and detach embeddings

                # Calculate both losses
                reconstruction_loss = multi_loss(y_, y)
                embedding_loss = F.mse_loss(pred_embeddings, target_embeddings)

                # Combine losses (you may want to add a weighting factor)
...
def save_last_weights(
    args: argparse.Namespace, model: torch.nn.Module, device_ids: List[int]
) -> None:
    """
...
def train_model(args: argparse.Namespace) -> None:
    ...
    # Load secondary model for embeddings
    secondary_model, secondary_config = get_model_from_config(
        args.secondary_model_type, args.secondary_config_path
    )

    # create a named tuple for the secondary model
    sec_args = argparse.Namespace(**vars(args))
    sec_args.start_check_point = args.secondary_checkpoint
    sec_args.model_type = args.secondary_model_type
    sec_args.config_path = args.secondary_config_path

    load_start_checkpoint(sec_args, secondary_model, type_="secondary")
    ...
    for epoch in range(config.training.num_epochs):
        ...
        best_metric = compute_epoch_metrics(
            model, args, config, device, device_ids, best_metric, epoch, scheduler
        )
