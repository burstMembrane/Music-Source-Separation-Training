...
# Audio-specific transforms (STFT, inverse-STFT)
...
class SpecDecoder(nn.Module):
    ...
    def forward(self, spec_magn, spec_angl, waveform_length=None):
        ...
        # Apply mask to phase before conversion
        spec_real = spec_magn * torch.cos(spec_angl)
        spec_imag = spec_magn * torch.sin(spec_angl)

        # Construct complex spectrogram
        spec = torch.complex(spec_real, spec_imag)  # [B, T, F]

        # Rearrange dimensions for ISTFT (inverse spectrogram)
        spec = spec.permute(0, 2, 1)  # From [B, T, F] to [B, F, T]

        # Perform the inverse spectrogram (ISTFT) to recover time-domain signal
