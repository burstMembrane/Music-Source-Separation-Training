...
from utils import apply_tta, demix, get_model_from_config, load_start_checkpoint
...
def type_1_main(
    ...
    def record_audio(stream_input: pyaudio.Stream) -> bytes:
        ...
        def check_stop_key(stop_flag) -> None:
...
if __name__ == "__main__":
    ...
    args.start_check_point = Path(args.start_check_point)
    args.out_dir = Path(args.out_dir)

    if not any(
        [
            args.config_path.exists(),
            args.start_check_point.exists(),
            args.out_dir.exists(),
        ]
    ):
        not_found_paths = [
            path
            for path in [args.config_path, args.start_check_point, args.out_dir]
            if not path.exists()
        ]
        print(f"Paths not found: {', '.join([str(path) for path in not_found_paths])}")
        exit(1)
